...
def get_classified_satellites(force_recompute: bool = False) -> pd.DataFrame:
    ...
    # Persist for quicker reads later
...
def filter_satellites(
    ...
    # Filter by purpose: if PURPOSE column exists, filter by it, otherwise search in OBJECT_NAME
    if purpose:
    ...
    # Build a response with expected semantic fields; use exact column names from CSV
    records: List[dict] = []
...
def persist_portal_request(payload: dict) -> str:
    ...
    # Create a human-readable TXT file
